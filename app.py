...
@st.cache_data
def load_model():
    coef_df = clean_dataframe(
        pd.read_excel(COEF_FILE, engine="openpyxl")
    )
    range_df = clean_dataframe(
        pd.read_excel(RANGE_FILE, engine="openpyxl")
    )
    cut_df = clean_dataframe(
        pd.read_excel(CUT_FILE, engine="openpyxl")
    )
    ...
    variable_col = next(
        (
            ...
            if column.lower() in {"variable", "var_name"}
        ),
        coef_df.columns[0],
    )
    ...
    coefficient_col = next(
        (
            column
            for column in coef_df.columns
            if column.lower() in {"coefficient", "coef", "beta"}
        ),
        coef_df.columns[1],
    )
    ...
    range_df["Low"] = pd.to_numeric(
        range_df["Low"],
        errors="coerce",
    )

    range_df["High"] = pd.to_numeric(
        range_df["High"],
        errors="coerce",
    )
    ...
    if cut_values.empty:
        raise ValueError(
            "No valid risk cutoff was found in risk_cut.csv."
        )

    return (
        coef_df,
        range_df,
        float(cut_values.iloc[0]),
    )

def get_range_row(variable, range_df):
    rows = range_df.loc[
        range_df["Variable"].eq(variable)
    ]
...
def get_laboratory_variables(coef_df):
    model_variables = get_model_variables(coef_df)

    # 按照指定顺序筛选模型中实际存在的变量
    laboratory_vars = [
        variable
        for variable in LABORATORY_DISPLAY_ORDER
        if variable in model_variables
        and variable in MEASUREMENT_VARIABLES
        and variable not in PHYSICAL_VARIABLES
    ]

    # 如果模型中有其他检验变量，也放在指定变量之后
    other_variables = [
        variable
        for variable in coef_df["Variable"]
        if variable in MEASUREMENT_VARIABLES
        and variable not in laboratory_vars
        and variable not in PHYSICAL_VARIABLES
    ]

    return laboratory_vars + other_variables
...
def get_user_inputs(coef_df, range_df):
    ...
    st.subheader(
        "Health behaviors and functional status"
    )
    ...
    laboratory_vars = get_laboratory_variables(
        coef_df
    )
...
def calculate_score(raw, coef_df, range_df):
    ...
    coefficient_map = coef_df.set_index(
        "Variable"
    )
    ...
    for group_name, variables in CATEGORICAL_GROUPS.items():
        ...
        for variable in included:
            contribution, _ = contribution_for(
                variable
            )
    ...
    for variable in coef_df["Variable"]:
        ...
        contribution, condition = contribution_for(
            variable
        )
        ...
        if variable == "Age":
            ...
            name = (
                "Smoking status: "
                f"{raw.get('_smoking_label', '')}"
            )

        elif variable == "marital_status":
            name = (
                "Marital status: "
                f"{raw.get('_marital_label', '')}"
            )

        elif variable == "Sex":
            name = (
                "Sex: "
                f"{'Male' if raw.get('Sex', 0) else 'Female'}"
            )

        elif variable == "SRH":
            name = (
                "Self-rated health: "
                f"{raw.get('_srh_label', '')}"
            )
...
def render_result_cards(score, threshold):
    ...
    st.caption(
        ...
        "The value 1.11 represents the 90th percentile of the "
        "risk score among older adults with cardiometabolic "
        "multimorbidity in Shenzhen."
    )
...
def main():
    ...
    raw, condition_count = get_user_inputs(
        coef_df,
        range_df,
    )
    ...
    st.caption(
        "The calculated risk score and risk category "
        "are for reference only and should not be used "
